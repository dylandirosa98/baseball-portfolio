...
export default function PartnerDashboardClient({ organization, role, athletes, paymentLinks, memberCount }: { organization: PartnerOrganizationRow; role: PartnerRole; athletes: Athlete[]; paymentLinks: PaymentLink[]; memberCount: number }) {
  const [tab, setTab] = useState<"athletes" | "payments" | "brand" | "domain" | "team">("athletes");
  ...
  const estimated = (organization.wholesale_billing_exempt ? 0 : (organization.partnership_type === "white_label" ? organization.white_label_monthly_cents : 0) + counts.pro * organization.pro_wholesale_cents + counts.elite * organization.elite_wholesale_cents) + athletes.filter((a) => a.has_custom_domain).length * organization.domain_wholesale_cents;
    ...
    <main className="min-h-screen bg-[#070b10] text-white" style={{ "--partner": organization.primary_color } as React.CSSProperties}>
      <header className="border-b border-white/10 bg-[#0a1119]">
        <div className="mx-auto flex min-h-20 max-w-[1500px] items-center justify-between gap-4 px-4 sm:px-7">
          ...
          <div className="flex items-center gap-2"><Link href={adminHost || "/dashboard"} className="rounded-lg border border-white/10 px-4 py-2.5 text-xs font-bold text-white/55 hover:text-white">My account</Link>{organization.wholesale_billing_exempt && <span className="rounded-full bg-violet-300/10 px-3 py-1.5 text-[10px] font-bold uppercase text-violet-200">Test account</span>}<span className="rounded-full border border-white/10 px-3 py-1.5 text-[10px] font-bold uppercase text-white/45">{organization.partnership_type.replace("_", " ")}</span></div>
        </div>
      </header>
      <div className="mx-auto max-w-[1500px] px-4 py-7 sm:px-7">
        ...
        {tab === "payments" && <section className="mt-6 grid gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-[#0b131c] p-6"><CreditCard className="h-6 w-6 text-[var(--partner)]" /><h2 className="mt-4 text-xl font-black">Wholesale billing</h2><p className="mt-2 text-sm leading-6 text-white/45">{organization.wholesale_billing_exempt ? "This internal test account is exempt from the platform fee and athlete wholesale charges. A billing card is only required if you purchase a real player domain." : "Your card pays Diamond Profile only for active partner-paid or customer-paid athlete seats. Canceled subscriptions automatically stop counting."}</p><button disabled={busy || !canAdmin} onClick={setupBilling} className="mt-5 rounded-lg bg-white px-4 py-3 text-sm font-black text-black">{organization.billing_payment_method_ready ? "Update billing card" : organization.wholesale_billing_exempt ? "Add card for domains" : "Add billing card"}</button><p className="mt-3 text-xs text-white/35">Status: {organization.wholesale_billing_exempt ? "Complimentary testing" : organization.billing_payment_method_ready ? "Ready" : "Setup required"} · {organization.platform_subscription_status}</p>{organization.billing_sync_error && <p className="mt-3 text-xs text-red-200">{organization.billing_sync_error}</p>}</div>
          ...
          {canAdmin && organization.stripe_account_status === "active" && <form action={(form) => run(() => api(`/api/partner/organizations/${organization.id}/payment-links`, { method: "POST", body: JSON.stringify({ name: form.get("name"), tier: form.get("tier"), url: form.get("url") }) }))} className="rounded-2xl border border-white/10 bg-[#0b131c] p-6 lg:col-span-2"><h2 className="font-black">Add verified Payment Link</h2><p className="mt-2 text-sm leading-6 text-white/40">Paste a recurring Payment Link created in your connected Stripe account. New athlete checkout links automatically use the selected tier.</p><div className="mt-4 grid gap-3 md:grid-cols-[180px_130px_1fr_auto]"><input name="name" required placeholder="Checkout name" className="rounded-lg border border-white/10 bg-black/25 px-3 py-3 text-sm" /><select name="tier" className="rounded-lg border border-white/10 bg-[#0b131c] px-3 py-3 text-sm"><option value="pro">Pro</option><option value="elite">Elite</option></select><input name="url" required placeholder="https://buy.stripe.com/..." className="rounded-lg border border-white/10 bg-black/25 px-3 py-3 text-sm" /><button disabled={busy} className="rounded-lg bg-white px-5 text-sm font-black text-black">Verify</button></div><div className="mt-5 grid gap-2">{paymentLinks.map((link) => <div key={link.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 px-3 py-2 text-xs"><span><Check className="mr-1 inline h-3 w-3 text-emerald-300" />{link.name} · {link.tier} · {link.unit_amount == null ? "custom price" : `${(link.unit_amount / 100).toFixed(2)} ${String(link.currency || "usd").toUpperCase()}`}</span><button type="button" disabled={busy} onClick={() => { if (window.confirm(`Disable ${link.name}? Existing subscriptions stay managed by Stripe, but new checkouts stop working.`)) void run(() => api(`/api/partner/organizations/${organization.id}/payment-links`, { method: "DELETE", body: JSON.stringify({ id: link.id }) })); }} className="inline-flex items-center gap-1 text-white/40 hover:text-red-200"><Trash2 className="h-3 w-3" />Disable</button></div>)}</div></form>}
        </section>}
        ...
        {tab === "domain" && <section className="mt-6 grid gap-6 lg:grid-cols-2">
          {organization.partnership_type === "white_label" ? <form action={connectDomain} className="rounded-2xl border border-white/10 bg-[#0b131c] p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-white/45">Use your apex domain, such as academybaseball.com. Management will run at <strong className="text-white/70">admin.yourdomain.com</strong>, the builder at <strong className="text-white/70">builder.yourdomain.com</strong>, and each athlete at <strong className="text-white/70">slug.yourdomain.com</strong>.</p>
            ...
            <button disabled={busy || !canAdmin} className="mt-4 rounded-lg bg-[var(--partner)] px-5 py-3 text-sm font-black">{organization.profile_domain ? "Recheck domain" : "Connect domain"}</button>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0b131c] p-6">
            ...
            {canAdmin && organization.stripe_account_status === "active" && <form action={(form) => run(() => api(`/api/partner/organizations/${organization.id}/payment-links`, { method: "POST", body: JSON.stringify({ create: true, name: form.get("name"), tier: form.get("tier"), priceCents: Math.round(Number(form.get("price")) * 100) }) }))} className="mt-5 rounded-lg border border-white/10 bg-black/15 p-4"><p className="text-xs font-bold uppercase tracking-[.14em] text-white/45">Create a checkout in your Stripe</p><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_110px_110px_auto]"><input name="name" required placeholder="Pro checkout" className="rounded-lg border border-white/10 bg-black/25 px-3 py-2.5 text-sm" /><select name="tier" className="rounded-lg border border-white/10 bg-[#0b131c] px-3 py-2.5 text-sm"><option value="pro">Pro</option><option value="elite">Elite</option></select><input name="price" required type="number" min={(organization.pro_wholesale_cents / 100).toFixed(2)} step="0.01" placeholder="15.00" className="rounded-lg border border-white/10 bg-black/25 px-3 py-2.5 text-sm" /><button disabled={busy} className="rounded-lg bg-white px-3 py-2.5 text-xs font-black text-black">Create</button></div><p className="mt-2 text-[11px] text-white/30">Retail price must be at least the platform base. The exact wholesale base for this organization is enforced server-side.</p></form>}
            <div className="mt-5 space-y-3 text-sm">
              ...
              {organization.partnership_type === "white_label" && <div className="flex items-center justify-between rounded-lg border border-white/10 px-4 py-3"><span className="text-white/55">White-label platform fee</span><strong>{organization.wholesale_billing_exempt ? "$0 test account" : "$200/mo"}</strong></div>}
