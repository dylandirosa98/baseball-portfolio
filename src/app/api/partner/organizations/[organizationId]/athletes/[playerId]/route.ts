...
export async function PATCH(request: NextRequest, context: { params: Promise<{ organizationId: string; playerId: string }> }) {
  ...
  try {
    if (action === "activate" && result.data.partner_billing_source === "partner_paid") {
      if (!access.organization.billing_payment_method_ready) return NextResponse.json({ error: "Set up partner billing first." }, { status: 409 });
      await admin.from("players").update({ partner_billing_status: "active", billing_tier: result.data.partner_plan, subscription_status: "active" }).eq("id", playerId);
      ...
      if (result.data.partner_billing_source === "partner_paid") {
        ...
        if (result.data.partner_stripe_subscription_id && access.organization.stripe_account_id && ["active", "past_due", "canceling"].includes(result.data.partner_billing_status)) {
          ...
          await admin.from("partner_customer_subscriptions").update({ tier: plan, payment_link_id: link.data.id }).eq("stripe_account_id", access.organization.stripe_account_id).eq("stripe_subscription_id", subscription.id);
        }
        await admin.from("partner_profile_checkouts").update({ active: false }).eq("player_id", playerId);
        await admin.from("partner_profile_checkouts").upsert({ organization_id: organizationId, player_id: playerId, payment_link_id: link.data.id, active: true }, { onConflict: "player_id,payment_link_id" });
        await admin.from("players").update({ partner_plan: plan, partner_payment_link_id: link.data.id, billing_tier: ["active", "past_due", "canceling"].includes(result.data.partner_billing_status) ? plan : "free" }).eq("id", playerId);
      }
      ...
      await admin.from("players").update({ partner_billing_status: "canceled", billing_tier: "free", subscription_status: "canceled", is_published: false }).eq("id", playerId);
    } else {
