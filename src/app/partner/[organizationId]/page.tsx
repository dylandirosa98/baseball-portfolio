...
export default async function PartnerDashboardPage({ params }: { params: Promise<{ organizationId: string }> }) {
  ...
  const [athletes, links, members] = await Promise.all([
    admin.from("players").select("id, first_name, last_name, invited_email, slug, partner_plan, partner_billing_source, partner_billing_status, billing_tier, is_published, has_custom_domain, updated_at, partner_profile_checkouts(token, active)").eq("organization_id", organizationId).order("created_at", { ascending: false }),
